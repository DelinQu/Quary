...
*/
// isInFunc是显示当前节点是否在功能块中的标志。此标志用于计算函数声明的localOffset。
static int isInFunc = FALSE;

/* mainFuncLoc is the location of main() function */
// mainFuncLoc是main（）函数的位置
...
// 函数getBlockOffset返回list所在块中临时变量的偏移量
 ...
 */
/*生成代码树的代码遍历程序。第二个参数（codefile）是代码文件的文件名，
用于将文件名打印为代码文件中的注释
*/
